...
class CategoryOption(models.Model):
    ...
    # The product picture (3:2 card art: brand logo + region flag + amount).
    # Normally written by `generate_option_images`; an admin upload overrides
    # it. The tile shows it in place of the small icon; the icon stays for
    # the dropdown and for options without a picture. Public storage only —
    # the URL lands in cached page HTML, so it must never expire.
    image = models.ImageField(
        upload_to='option_images/', blank=True, null=True,
        storage=get_public_media_storage,
        help_text='Product picture (3:2). Generated by generate_option_images; '
