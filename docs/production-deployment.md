...
- `GUARD_EMAIL_IMAP_*` enable the email guard type for offline-activation accounts: one dedicated mailbox (never a personal one — these credentials read it) receives the Steam Guard emails for every email-guard account; codes are matched to accounts by the login named in the email body. For Gmail use an app password (requires 2FA on the Google account). Leave unset to disable — mobile-authenticator (`totp`) accounts work regardless.
